...
// We no longer import this, as the call will be made to our secure API
...
export default function ChatBot() {
  ...
  const [messages, setMessages] = useState([
    ...
    { sender: "bot", text: "Please ask queries only related to Shyam Lal College's Innovation Council." },
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      // Call our own secure backend API route instead of Gemini directly
    ...
    <div className="fixed bottom-6 right-6 z-50">
      {isOpen ? (
          ...
          <div className="flex-1 overflow-y-auto p-4 bg-gradient-to-b from-gray-50 to-white space-y-3">
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-xs sm:max-w-md rounded-lg px-4 py-2 ${msg.sender === "bot" ? "bg-gray-100 text-gray-800 rounded-tl-none" : "bg-orange-600 text-white rounded-tr-none"}`}>
                  {msg.text}
                </div>
              </div>
            ))}
        ...
        <div className="animate-float will-change-transform">
          <button onClick={toggleChat} className="rounded-full transition-all focus:outline-none focus:ring-2focus:ring-offset-2" aria-label="Open chatbot">
            <DotLottieReact src="https://lottie.host/6a32f311-2b33-4423-ab38-1b96546ae297/Ft702URpxi.lottie" loop autoplay style={{ width: "90px", height: "90px" }} />
          </button>
        </div>
      )}
    </div>
